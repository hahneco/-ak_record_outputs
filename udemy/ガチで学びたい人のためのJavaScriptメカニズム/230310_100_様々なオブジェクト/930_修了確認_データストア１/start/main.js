...
class DataSource {
  // データ取得処理
  static getLocal(KEY) { // staticメソッド
...
/*
// データ取得処理
function getLocal(KEY) { // データストレージを見て何かデータがあればオブジェクトとしし使用する
  const result = localStorage.getItem(KEY);
  return JSON.parse(result);
}
*/
...
// 初期値の設定
// getLocalがtrue（値がとれた）の場合、targetobjに格納。
// getLocalがtrueに値がなければ空のオオブジェクトをセット
const targetobj = DataSource.getLocal(KEY) || {};

const pxy = new Proxy(targetobj, { // 第二引数はハンドラー
  set(target, prop, value, receiver) { // setで値が変更された時にトラップする // pxyがreceiverに入る
    const result = Reflect.set(target, prop, value, receiver);

    DataSource.setLocal(KEY, target); // localStrageに値ほ保存。オブジェクトをJSONに直す
